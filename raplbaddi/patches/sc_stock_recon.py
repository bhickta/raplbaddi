...
class StockReconciliationManager:
    ...
    def cancel_and_create_reconciliation(self, stock_reconciliation_id, new_date):
        try:
            ...
            print(f"Error processing {stock_reconciliation_id}: {str(e)}")
    ...
    def create_amended_reconciliation(self, stock_reconciliation, items, posting_date):
        ...
        self.change_warehouse_in_dnpr(sc_details, posting_date)
    ...
    def change_warehouse_in_dnpr(self, sc, posting_date):
        self.start_date = posting_date
        self.end_date = frappe.utils.today()
        dnpr = frappe.get_all("Delivery Note", fields=["name"], filters={"customer": sc.customer, "posting_date": ["between", [self.start_date, self.end_date]], "docstatus": 1})
        for dn in dnpr:
            dn = frappe.get_doc("Delivery Note", dn.name)
            dn.cancel()
            changed_fields = {"posting_date": dn.posting_date, "set_posting_time": True}
            rows_updated = {"items": dn.items}
            new_dn = self.amend_document(dn, changed_fields, rows_updated, submit=True)
            print(new_dn)
            try:
                new_dn.submit()
            except erpnext.exceptions.PartyDisabled as e:
                frappe.db.sql(f"UPDATE `tabCustomer` SET disabled = 0 WHERE name = '{sc.customer}'")
                new_dn.submit()
                frappe.db.sql(f"UPDATE `tabCustomer` SET disabled = 1 WHERE name = '{sc.customer}'")
